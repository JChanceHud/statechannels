...
describe('signatures', () => {
  describe('signState', () => {
    it('signs a state', async () => {
      ...
      const state: State = {
        channel: {channelNonce: 0x01, participants: [wallet.address]},
        outcome: [],
        turnNum: 1,
        isFinal: false,
        appData: '0x00',
  ...
  describe('signChallengeMessage', () => {
    it('signs a challenge message', async () => {
      const wallet = Wallet.createRandom();
      const channel = {channelNonce: 0x01, participants: [wallet.address]};
      const state: State = {
        channel,
        outcome: [],
        turnNum: 1,
  ...
  describe('getStateSignerAddress', () => {
    it('correctly recovers a state signer address', async () => {
      const wallet = Wallet.createRandom();
      const state: State = {
        channel: {channelNonce: 0x1, participants: [wallet.address]},
        outcome: [],
        turnNum: 1,
        isFinal: false,
        appData: '0x00',
